...
This variant renders montages using the PSNR-nearest real slices (with PSNR
labels), rather than the SSIM-nearest ones. The CSV still contains both metrics.
...
def main():
    ...
    ap.add_argument("--out_dir", default="memorization_out_psnr")
    ...
    # ---- montages: generated + its topk PSNR-nearest reals ----
    def mag_from_mm(ix):
    ...
    for gi in range(n_gen):
        ...
        axes[0].axis("off")
        for r, (sc, ix) in enumerate(best_psnr[gi]):
    ...
    # ---- quick summary: the most-suspicious matches (by PSNR) ----
    top_overall = []
    for gi in range(n_gen):
        if best_psnr[gi]:
            top_overall.append((best_psnr[gi][0][0], gi, best_psnr[gi][0][1]))
    top_overall.sort(reverse=True)
    print("\nMost similar generated->real matches (by PSNR):")
    for sc, gi, ix in top_overall[:10]:
        print(f"  generated {gi:3d}  ->  real {ix:6d}   PSNR {sc:.2f} dB")
